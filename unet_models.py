...
class UNet11_two_branch_out(nn.Module):
    def __init__(self, num_filters=32, pretrained=False):
        ...
        self.dec4_2 = DecoderBlock(
            num_filters * (16 + 8), num_filters * 8 * 2, num_filters * 4
        )
        self.dec3_2 = DecoderBlock(
            num_filters * (8 + 4), num_filters * 4 * 2, num_filters * 2
        )
        self.dec2_2 = DecoderBlock(
            num_filters * (4 + 2), num_filters * 2 * 2, num_filters
        )
        self.dec1_2 = ConvRelu(num_filters * (2 + 1), num_filters)
    ...
    def forward(self, x):
        ...
        out1 = self.final_1(dec1)

        # second branch
        dec5_2 = self.dec5_2(torch.cat([center, conv5], 1))
        dec4_2 = self.dec4_2(torch.cat([dec5_2, conv4], 1))
        dec3_2 = self.dec3_2(torch.cat([dec4_2, conv3], 1))
        dec2_2 = self.dec2_2(torch.cat([dec3_2, conv2], 1))
        dec1_2 = self.dec1_2(torch.cat([dec2_2, conv1], 1))
        out2 = self.final_2(dec1_2)
