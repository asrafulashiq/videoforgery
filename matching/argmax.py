...
def _make_radial_window(width, height, cx, cy, fn, window_width=10.0):
    ...
    xs = torch.arange(width).view((1, width)).repeat(
        channels, height, 1).float().cuda()
    ys = torch.arange(height).view((height, 1)).repeat(
        channels, 1, width).float().cuda()
    delta_xs = xs - cx
    delta_ys = ys - cy
...
class SoftArgmax1D(torch.nn.Module):
    ...
    def __init__(self, base_index=0, step_size=1, wt=5):
        """
...
class SoftArgmax2D(torch.nn.Module):
    ...
    def __init__(self, base_index=0, step_size=1, window_fn=None, window_width=10,
                 wt=2, softmax_temp=1.0):
        """
        The "arguments" are base_index, base_index+step_size, base_index+2*step_size, ... and so on for
        arguments at indices 0, 1, 2, ....
    ...
    def _softmax_2d(self, x, temp):
        ...
        x_softmax = self.softmax(self.wt * x_flat)
    ...
    def forward(self, x):
        ...
        argmax_x, argmax_y = torch.remainder(argmax, width).float(
        ), torch.floor(torch.div(argmax.float(), float(width)))
        windows = _make_radial_window(
            width, height, argmax_x, argmax_y, self.window_fn, self.window_width)
        windows = windows.view(batch_size, channels,
                               height, width).to(x.device)
        smax = self._softmax_2d(x, self.softmax_temp) * windows
        smax = smax / torch.sum(smax.view(batch_size, channels, -1),
                                dim=2).view(batch_size, channels, 1, 1)
        ...
        x_indices = torch.arange(
            start=self.base_index, end=x_end_index, step=self.step_size).to(smax.device).type_as(smax)
        ...
        y_indices = torch.arange(
            start=self.base_index, end=y_end_index, step=self.step_size).to(smax.device).type_as(smax)
